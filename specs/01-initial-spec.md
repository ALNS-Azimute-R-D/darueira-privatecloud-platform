...
## 11. Implementation Status & Known Deviations (verified 2026-09-22)
...
| Cilium CNI | **Live (cutover done 2026-09-22)** | Calico fully removed, Cilium `1.15.2` + Hubble Relay/UI running (`docs/runbooks/cilium-cni-migration.md`, Fase 0-2). **Default-deny is live and enforced** on `corpshared-{plat,mgmt,obs,secr-internal}` — these 4 `CiliumNetworkPolicy` objects were already applied 35 days before the cutover and began enforcing the instant Cilium started, with no audit-mode step; a real bug in the `corpshared-obs` policy (missing `toEntities: [cluster, world]` egress fallback) broke DNS for `prometheus`/`fluent-bit` and was found and fixed live. The 5th policy (tenant baseline) is still **not applied** to any `drr-tnt-*` namespace. `L3-L7 policies, WireGuard, kube-proxy replacement` remain **not running** (target-only) |
...
1. **Network isolation is declared but not enforced.** ~~Either enable Cilium (per ADR-0004) or replace the `CiliumNetworkPolicy` objects with standard `NetworkPolicy` objects that Calico enforces.~~ **Mostly resolved 2026-09-22**: Cilium CNI cutover done and default-deny is live/enforced on the 4 control-plane namespaces (`corpshared-{plat,mgmt,obs,secr-internal}`) — see `docs/runbooks/cilium-cni-migration.md`. Remaining: roll out the tenant policy (`tnt-tenant-base/network-policy-template.yaml`) in audit mode (Fase 3 of the runbook) — it likely has the same missing-`toEntities`-fallback bug found and fixed in the `corpshared-obs` policy, so treat it as untested rather than assuming it will work as-is.
