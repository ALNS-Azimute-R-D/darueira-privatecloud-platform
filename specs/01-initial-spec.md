...
| Cilium CNI | **Live (cutover done 2026-09-22)** | Calico fully removed, Cilium `1.15.2` + Hubble Relay running (`docs/runbooks/cilium-cni-migration.md`, Fase 0-1). `L3-L7 policies, WireGuard` still **not running**: the 5 `CiliumNetworkPolicy` objects exist but are not yet applied (Fase 3-4 of the runbook, still pending). Namespace default-deny is therefore not active yet |
...
1. **Network isolation is declared but not enforced.** ~~Either enable Cilium (per ADR-0004) or replace the `CiliumNetworkPolicy` objects with standard `NetworkPolicy` objects that Calico enforces.~~ **Partially resolved 2026-09-22**: Cilium CNI cutover done (see `docs/runbooks/cilium-cni-migration.md`). Remaining: roll out the 5 `CiliumNetworkPolicy` objects in audit mode per namespace, then enable enforcement (Fase 3-4 of the runbook).
